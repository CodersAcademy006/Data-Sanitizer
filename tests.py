...
def simple_json_data():
    ...
    return {
        "user": {"name": "John Doe", "address": {"street": "123 Main St", "city": "New York"}},
        "items": ["a", "b", "c"],
        "score": 95,
    }
...
class TestCleaningFunctions:
    ...
    def test_category_alias_map(self, dirty_dataframe):
        ...
        alias_map = build_category_alias_map(dirty_dataframe["status"], similarity_threshold=0.8)
    ...
    def test_small_csv_cleaning(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            cleaned_path, report_path = run_full_cleaning_pipeline_two_pass_sqlite_batched(
                path=csv_path, output_dir=output_dir, sqlite_path=os.path.join(tmpdir, "test.db"), chunksize=2
            )
...
class TestStorageBackend:
    ...
    def test_create_and_fetch_job(self):
        ...
        # Create job
        job_id = backend.create_job(tenant_id="test-tenant", dataset_name="test-dataset")
