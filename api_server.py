...
def check_rate_limit(tenant_id: str):
    ...
    if calls > max_calls_per_min:
        raise HTTPException(status_code=429, detail=f"Rate limit exceeded: {calls}/{max_calls_per_min} calls/min")
...
async def ingest_dataset(
    ...
    background_tasks.add_task(
        process_job, job_id=job_id, tenant_id=tenant_id, file_path=temp_file_path, pii_strategy=request.pii_strategy
    )
...
async def get_cleaning_report(job_id: str, api_key: str = Header(None)) -> CleaningReport:
    ...
    if job.get("status") != "success":
        raise HTTPException(status_code=400, detail=f"Job is in {job.get('status')} state, not completed")
...
async def download_cleaned_data(job_id: str, format: str = "parquet", api_key: str = Header(None)):
    """
    ...
    if job.get("status") != "success":
        raise HTTPException(status_code=400, detail="Cleaned data not available until job completes")
...
async def get_audit_log(job_id: str, filter_type: Optional[str] = None, api_key: str = Header(None)):
    """
...
async def get_confidence_scores(
    job_id: str, min_confidence: float = 0.0, max_confidence: float = 1.0, api_key: str = Header(None)
):
    """
    ...
    return {"job_id": job_id, "confidence_scores": [], "total_cells": 0, "low_confidence_count": 0}
...
async def get_metrics(api_key: str = Header(None)):
    ...
    return {"total_jobs": 0, "total_rows_processed": 0, "average_latency_seconds": 0.0, "success_rate": 1.0}
