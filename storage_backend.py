...
class StorageBackend:
    ...
    def _init_milvus(self):
        ...
        try:
            # Connect to Milvus
            connections.connect(alias=self.milvus_alias, host=self.milvus_config.host, port=self.milvus_config.port)
        ...
        with self.pg_connection() as conn:
            with conn.cursor() as cur:
                ...
                for i in range(0, len(hashes), BATCH_SIZE):
                    ...
                    cur.execute(
                        f"SELECT hash FROM row_hashes WHERE job_id = %s AND hash IN ({placeholders})", [job_id] + batch
                    )
...
def create_storage_backend(
    ...
    pg_config = PostgresConfig(host=pg_host, port=pg_port, database=pg_db, user=pg_user, password=pg_password)
